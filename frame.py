...
def refresh():
    ...
    def click1_1():
        frame4_pack()
        frame4.config(text=' 지역별 신규 확진자 발생비율 ')
        ...
        for i in range(0, len(sub_list[0])):
            if c == 4:
                c = 0
                s = s + 2
            label = Label(frame4, width=12, text=sub_list[0][i] + ' ' + sub_list[1][i], font=('', 12), borderwidth=0)
            label.grid(row=s, column=c, pady=(2, 0))
            label = Label(frame4, width=12, text=sub_list[1][i], font=('', 12), borderwidth=0)
    ...
    def click1_2():
        ...
        for i in range(0, len(sub_list[0])):
            if c == 5:
                c = 0
                s = s + 2
            label = Label(frame4, width=6, text=sub_list[0][i], font=('', 12), borderwidth=0)
            label.grid(row=s, column=c, padx=3, pady=(6, 0))
            label = Label(frame4, width=6, text=sub_list[1][i], font=('', 12), borderwidth=0)
        ...
        for i in range(0, len(seoul)):
            for j in range(0, len(seoul[0])):
                if i == 0:
                    ...
                    if j == 1:
                        button.config(command=click1_2)
                    # if j == 2:
                    #     button.config(command=click1_3)
                    # if j == 3:
                    #     button.config(command=click1_4)
                    # if j == 4:
                    #     button.config(command=click1_5)
